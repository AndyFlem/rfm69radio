...
RFM69.prototype.initialize = function({
  ...
  scope._resetRadio(function() {
    scope._checkSync(function() {
      ...
      scope._setEncryption(scope.encryptionKey,function(){
        ...
        scope._waitReady(function() {
          scope._setMode(reg.RF69_MODE_RX,function(){
...
RFM69.prototype.registerPacketReceivedCallback = function(packetReceivedCallback) {
  
  if(typeof packetReceivedCallback==='function'){
    this._packetReceivedCallbacks.push(packetReceivedCallback);
    debug(`********** New packetReceivedCallback registered (${this._packetReceivedCallbacks.length})`)
  }  
}
...
RFM69.prototype.readTemperature = function(callback, calFactor = 0) {
  ...
  const scope=this;
  this._setMode(reg.RF69_MODE_STANDBY,function(){
    scope._writeRegSync(reg.REG_TEMP1, reg.RF_TEMP1_MEAS_START);
    ...
    const wait = setInterval(function() {
      if (!(scope._readRegSync(reg.REG_TEMP1) & reg.RF_TEMP1_MEAS_RUNNING)) {
        ...
        scope._setMode(curMode,function(){
          callback(temp);
        });
      }
    }, 50);
...
RFM69.prototype.send = function({ toAddress = 0, payload = '', attempts = 3, attemptWait = 200, requireAck = true, ackCallback,}) {
  ...
  if (requireAck){
    ...
    scope._sendFrame(toAddress, payload, false,function(){
      peer.sending=false;
      ackCallback(null, 1);
    });
  }
...
RFM69.prototype._sendFrame = function(toAddress, payload, requestAck, callback) {
  ...
  this._setMode(reg.RF69_MODE_STANDBY,function() {
    ...
    scope._setMode(reg.RF69_MODE_TX,function(){
      debug(`Sent: ${payload}`);
      callback();
    });
  });
...
RFM69.prototype._setEncryption = function(key,callback) {
  const curMode = this.mode;
  const scope=this;
  this._setMode(reg.RF69_MODE_STANDBY,function(){
    if (key != 0 && key.length == 16) {
      ...
      scope._radio.transferSync(message);
      scope._writeRegSync(reg.REG_PACKETCONFIG2, (scope._readRegSync(reg.REG_PACKETCONFIG2) & 0xFE) | reg.RF_PACKET2_AES_ON);
    } else {
      debug(`Not setting encryption key: ${key}`);
      scope._writeRegSync(reg.REG_PACKETCONFIG2, (scope._readRegSync(reg.REG_PACKETCONFIG2) & 0xFE) | reg.RF_PACKET2_AES_OFF);
    }
    scope._setMode(curMode,function(){
      callback();
    });
...
RFM69.prototype._setMode = function(newMode, callback) {
  ...
  if (newMode!=this.mode){
    ...
    //    pass  
    this.mode = newMode;
    //console.log(`${formatDatetime(new Date())} Mode set to: ${this.modeName} (${this.mode})`);
...
RFM69.prototype._interruptHandler = function() {
  ...
  this._setMode(reg.RF69_MODE_STANDBY,function(){
    ...
    if (irqFlags & reg.RF_IRQFLAGS2_PACKETSENT) {
      ...
      scope._dataReceivedHandler(function(err,packet) {
        scope._setMode(reg.RF69_MODE_RX,function(){
          if (packet){
            for (let cb of scope._packetReceivedCallbacks) {
              cb(packet);
            }
          }
        });
